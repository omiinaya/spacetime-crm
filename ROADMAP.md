...
| **Test coverage** | Rust runtime tests (need STDB host), full-suite CI wiring | **~2h** | ✅ DONE (2026-07-31) — `test.yml` gains `test-container` job: boots STDB container, runs `scripts/run-integration-tests.sh` (pytest + Rust container tests) on push/PR to main |
...
| 2 | **Test isolation** — tests share STDB state; no fresh DB per run; partial fix exists (timestamped names) | ROADMAP 7G; `conftest.py` shared admin session | 2h | ✅ **Stale-session purge** — `_cleanup_stale_test_tenants()` runs pre-session, deleting ALL leftover `test-tenant-%` tenants (cascade) from interrupted runs; verified 25 stale tenants → 0. Fixed the concrete bug it caused: `_create_appointment` returned a foreign stale "Status Test" row. Lookup helpers now tenant-scope via `_jwt_tenant_id`. |
| 3 | **Rust runtime tests** — 211 `#[test]` are compile-only for wasm; container infra exists (`docker-compose.test.yml`, `make test-container`) but not wired into CI | `scripts/run-integration-tests.sh` exists; CI `test.yml` doesn't call it | 1h | ✅ **Host unit tests wired into CI** — `cargo test --lib` (208 tests, in-memory stub datastore) + `cargo clippy -D warnings` are now hard CI gates (were `continue-on-error`); clippy `--all-targets` cleaned to zero warnings. **Full container suite wired 2026-07-31** — `test.yml` gains `test-container` job (boots STDB container on :3002, runs `scripts/run-integration-tests.sh`: pytest + Rust container tests); Makefile `test-container` gate fixed to actually prefer the container runner (old gate tested a Docker image that doesn't exist, so it silently ran the standalone script whenever `spacetimedb-standalone` was on PATH) |
| 4 | **Offline mode (8B-9)** — PWA has service worker precache but no offline data strategy | `vite-plugin-pwa` config only precaches static assets | 4h+ | ✅ **Read-through cache + SW runtime caching** — API GETs cached in localStorage (5-min TTL) with stale-while-revalidate fallback on network failure; custom `public/sw.js` fetch handler does NetworkFirst for `/api/` + navigations and cache-first for assets (push notifications preserved); `useNetworkStatus` hook + amber offline banner; cache cleared on logout (no cross-account leakage). 6 cache unit tests. |
...
## 🔎 Review Addendum (2026-07-31 worker audit — feat/split-large-files)

Independent worker-agent review of the working tree. Verified by grep/AST/test runs, not README claims. Server + STDB healthy; 1,275 backend unit tests pass; ruff clean. Note: another agent was mid-refactor on this branch during the audit (48 in-flight files, stub cleanup `e972fd8`) — findings below are stable regardless.

### 🔴 Concrete open tasks (verified, in priority order)

| # | Task | Evidence | Effort | Status |
|---|------|----------|--------|--------|
| 1 | **`convert_estimate_to_invoice` drops currency** — `server/spacetimedb/src/estimate.rs:167` hardcodes `currency: "USD".to_string()` when creating the invoice from an estimate. `Estimate` HAS a `currency` field (line 22), so a EUR estimate converts to a USD invoice silently. Fix: `currency: est.currency.clone()`. One-line Rust change + wasm rebuild + publish + backend test. | `sed -n '120,170p' server/spacetimedb/src/estimate.rs` | 20m | ✅ DONE (e8e5508) — `est.currency.clone()` + `test_convert_estimate_preserves_currency` |
| 2 | **Recurring-invoice generation hardcodes currency** — `server/spacetimedb/src/recurring_invoice_rule.rs:162` inserts invoices with `currency: "USD".to_string()`. `RecurringInvoiceRule` struct + `RecurringInvoiceRuleCreate` Pydantic model have NO currency field (verified), so auto-generated invoices are always USD even in multi-currency tenants. Fix requires full layer plumbing: add `#[default("USD".to_string())] currency` at END of rule struct, reducer param, Pydantic field, frontend TS interface + form, then `printf 'y\ny\n' |` publish + grep all `_call("create_recurring_invoice_rule"` sites (routes + seed script). | grep above; skill `spacetime-crm-development` add-column pattern | 45m | ✅ DONE (e8e5508) — currency plumbed STDB→Python→frontend; NOTE: String defaults aren't const-eval in bindings, so column is `Option<String>` + `#[default(None::<String>)]`; added `helpers._sql` Option-unwrap so API returns plain string; published data-safe (no --delete-data) |
| 3 | **Root `server/models.py` (23KB, 77 classes) is shadowed dead code** — the `server/models/` package fully re-exports all 77 classes + `BaseModel` (AST-verified: `root_classes ⊆ pkg_names`), and `import models` resolves to the package (`server/models/__init__.py`). Only consumer is `server/tests/unit/test_models.py` (78KB), which loads the OLD root file via importlib bypass (`test_models.py:24-29`). Fix: re-point that test at the package (`import server.models as models` — identical API), delete root `server/models.py`, run `pytest tests/unit/test_models.py`. Docker `COPY server/ .` picks up the root file today (harmless bloat). | `python3 -c "import models; print(models.__file__)"` → package; AST class diff = 0 missing | 30m | ✅ DONE (e8e5508) — root `models.py` deleted via `git rm`, test re-pointed at package, 77/77 classes verified |
| 4 | **28 Rust placeholder tests** — `#[test] fn test_*_basic() { /* TODO: implement basic test */ assert!(true); }` across 18 files (appointment, audit, checklist, custom_field, customer, customer_geolocation, geolocation_tool, payment, pos, product, purchase_order, push_subscription, sla_config, tax_rate, tenant, ticket, user, webhook). They inflate the "211 Rust tests" claim and assert nothing. Fix: replace with real assertions or delete; keep `cargo clippy -D warnings` + `cargo build --release --target wasm32-unknown-unknown` green. | `grep -c "TODO: implement basic test" server/spacetimedb/src/*.rs` = 28 | 1h | ✅ DONE (e8e5508) — 16 redundant placeholders deleted (real `*_test.rs` siblings exist), real tests written for `geolocation_tool` (2) + `push_subscription` (5); suite now 227 Rust tests |
| 5 | **`asyncio_default_fixture_loop_scope` unset** — pytest-asyncio emits a deprecation warning on every run; future default changes to function-scope and may break session-scoped async fixtures in `tests/conftest.py`. Fix: add `asyncio_default_fixture_loop_scope = "function"` to `[tool.pytest.ini_options]` in `pyproject.toml`, re-run full suite to confirm no scope breakage. | warning text in every pytest run | 10m | ✅ DONE (e8e5508) |
| 6 | **`print()` in production path** — `server/main.py:35` uses `print()` for scheduler startup instead of `logging`. Fix: `logger.info(...)`. Cosmetic but matches `log_config.py` structured logging. | `sed -n '30,40p' server/main.py` | 5m | ✅ DONE (e8e5508) — `logger.info("[scheduler] Started: %s (every %ss)", ...)` |

### Verified NOT issues (don't burn time)
- `server/test_*.py` root files vs `server/tests/test_*.py` are NOT duplicates: root files are the canonical unit tests; `server/tests/` versions are live-server integration wrappers that `import *` them and skip when the server is down (`server/tests/test_helpers.py:14-27`). Intentional pattern.
- Rust `unwrap()` calls in `gift_card.rs`/`test_stubs.rs` are all inside `#[cfg(test)]` blocks — test-only, not production risk.
- The 11 assert-True stub files found at audit start were already removed by the concurrent agent (`e972fd8`).
- STDB standalone (2.6.1) and CRM server are healthy; `docker-compose.yml` already pins `clockworklabs/spacetime:v2.6.1`.
