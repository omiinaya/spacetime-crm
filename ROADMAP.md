...
- **POS receipt PDF — ADDED** — `GET /api/pos/sales/{sale_id}/receipt-pdf` generates a printable receipt PDF using Jinja2 + Chromium. Courier-style template with store info, line items (qty/price/total), subtotal/discount/tax/total, payment method/tendered/change. Frontend: "PDF" button on receipt view opens the PDF in a new tab. 4 new backend tests.
