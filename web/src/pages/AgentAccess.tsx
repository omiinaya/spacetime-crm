...
export default function AgentAccessPage() {
  ...
  const handleApprove = async (agent: HermesIdAgent) => {
    if (!window.confirm(`Approve agent "${agent.display_name || agent.did}" for project access?`))
      return;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Approve or deny hermes-id AI agents requesting access to this project
          </p>
        </div>
          ...
          {!loading && !error && agents.length === 0 && (
            <div className="py-16 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                No pending agent requests. New agents requesting access will appear here.
              </p>
            </div>
          )}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-left text-xs uppercase tracking-wide text-muted-foreground">
                    ...
                    <th className="py-2 pr-3 font-medium">Requested Projects</th>
                  ...
                  {agents.map((agent) => (
                    ...
                    >
                      <td className="py-3 pr-3">
                        <span className="font-mono text-xs break-all">{agent.did}</span>
                      </td>
                      <td className="py-3 pr-3">
                        {agent.display_name || <span className="text-muted-foreground">—</span>}
                      </td>
