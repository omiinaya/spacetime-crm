...
function SmsSettingsSection() {
	...
	return (
		<Card>
			...
			<CardContent className="space-y-4">
				<p className="text-sm text-muted-foreground">
					Configure Twilio to send SMS notifications to customers when tickets
					are updated, invoices are created, payments are received, appointments
					are scheduled, or estimates are approved. Messages are sent to the
					customer's <strong>mobile</strong> phone number.
				</p>
				...
				<div className="flex gap-2">
					<Button onClick={() => saveMutation.mutate(smsConfig)}>
						Save Settings
					</Button>
					<Button
						variant="outline"
				...
				<div className="border-t pt-3">
					<p className="text-xs text-muted-foreground">
						<strong>Notification triggers:</strong> Ticket status changes, new
						invoices, payment receipts, appointment scheduling, and estimate
						approvals will automatically send an SMS to the customer's mobile
						number when Twilio is configured.
					</p>
				</div>
			</CardContent>
		</Card>
					...
					<div>
						...
						{!showDisable ? (
							...
							<div className="space-y-3">
								<p className="text-xs text-muted-foreground">
									Enter a current 2FA code from your authenticator app to
									disable.
								</p>
								<div className="flex gap-2">
									<Input
