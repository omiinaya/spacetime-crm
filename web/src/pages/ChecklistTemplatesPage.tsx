...
function TemplateEditor({ template, onSave, onCancel }: TemplateEditorProps) {
  ...
  const [items, setItems] = useState<ChecklistItem[]>(() => {
    if (template) {
      try {
        return JSON.parse(template.items);
      } catch {
        return [];
      }
    }
...
export default function ChecklistTemplatesPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: { id: string; name: string; description: string; items: ChecklistItem[] }) =>
      api.checklist.templates.update(data.id, {
        name: data.name,
  ...
  return (
    <div className="space-y-6">
      ...
      {(showEditor || editing) && (
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">{editing ? 'Edit Template' : 'New Template'}</CardTitle>
          </CardHeader>
          <CardContent>
            <TemplateEditor
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p className="text-xs mt-1">
            Create templates to quickly apply standard checklists to tickets
          </p>
        </div>
      ) : (
                ...
                <CardContent className="p-4">
                  ...
                  {done > 0 && (
                    <div className="mt-2 space-y-1">
                      ...
                      {done > 4 && (
                        <p className="text-xs text-muted-foreground mt-1">+{done - 4} more</p>
                      )}
                    </div>
                  )}
                </CardContent>
