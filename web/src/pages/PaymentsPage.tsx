...
export default function PaymentsPage() {
  ...
  // Derive currency from the most common payment currency, falling back to form value
  ...
  const totalCurrency =
    Object.entries(currencyCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || form.currency || 'USD';
        ...
        <CardContent className="pt-4 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-green-400">
              {totalCurrency} {totalAmount.toFixed(2)}
            </p>
          </div>
        </CardContent>
      </Card>
            ...
            >
              ...
              {invoices.map((inv) => (
                <option key={inv.id} value={inv.id}>
                  #{inv.invoice_number} — {inv.currency || 'USD'} {inv.total.toFixed(2)} (
                  {inv.status})
                </option>
              ))}
            </Select>
              ...
              <CardContent className="pt-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    via {p.method} — {new Date(p.created_at).toLocaleDateString()}
                  </p>
                  {p.reference && (
