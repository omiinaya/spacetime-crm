...
export default function PosPage() {
	...
	const { data: searchResults } = useQuery({
		...
		queryFn: () =>
			api.products
				.list(searchQuery, undefined, undefined, 0, 20)
				.then((r) => r.products),
	...
	const handleSaleComplete = useCallback(async () => {
		...
		if (paymentMethod === "gift_card") {
			try {
				await api.giftCards.redeem({
					code: giftCardCode.trim(),
					amount: total,
				});
			} catch {
				toast.error(
					"Gift card redemption failed — sale created but payment not collected",
				);
			...
			if (e.key === "Enter") {
				...
				api.products
					...
					.catch(() => {
						...
						api.products
							.list(barcode, undefined, undefined, 0, 5)
							.then((res) => {
				...
				<div className="flex items-center gap-4 mb-6">
					...
					{sale.status !== "refunded" && sale.status !== "voided" && (
						<Button
							...
							disabled={refunding}
							onClick={async () => {
								if (!confirm("Refund this sale? This cannot be undone."))
									return;
								setRefunding(true);
								try {
								...
								<div>
									...
									<div className="flex gap-1">
										...
										<Button
											size="sm"
											variant={
												paymentMethod === "gift_card" ? "default" : "outline"
											}
											className="flex-1 text-xs"
										...
										<div className="mt-2 space-y-2">
											<div className="flex gap-1">
												...
												<Button
													...
													disabled={
														giftCardBalanceLoading || !giftCardCode.trim()
													}
												>
											...
											{giftCardBalance !== null && giftCardBalance < total && (
												<p className="text-xs text-red-500">
													...
													{(total - giftCardBalance).toFixed(2)} more
												</p>
											)}
										</div>
									)}
