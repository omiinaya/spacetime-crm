...
export default function GiftCardsPage() {
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`Gift card created: ${res.gift_card.code} — $${amount.toFixed(2)}`);
        setCreateAmount('');
        setCreateCustomer('');
        setShowCreate(false);
        queryClient.invalidateQueries({ queryKey: ['gift-cards'] });
            ...
            <div className="flex gap-2">
              ...
              <div className="flex items-end">
                <Button
                  ...
                  disabled={createLoading || !createAmount || parseFloat(createAmount) <= 0}
                >
                  {createLoading ? (
          ...
          {lookupResult && (
            <div className="mt-3 border rounded-lg p-3 space-y-1 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <Badge variant={lookupResult.active ? 'default' : 'destructive'}>
              ...
              {lookupResult.expires_at > 0 && (
                <div className="flex justify-between">
                  ...
                  <span>{new Date(lookupResult.expires_at).toLocaleDateString()}</span>
                </div>
              )}
              {lookupResult.notes && (
                <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span>{new Date(lookupResult.created_at).toLocaleString()}</span>
              </div>
            </div>
          )}
        </CardContent>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant={card.active ? 'default' : 'destructive'} className="text-xs">
                        {card.active ? 'Active' : 'Voided'}
                      </Badge>
                    </div>
                    <div className="text-xs text-muted-foreground mt-1 flex gap-4">
                      <span>
                        Customer: <span className="font-medium">{card.customer_name || '—'}</span>
                      </span>
                      <span>
                        Created:{' '}
                  ...
                  <div className="text-right flex items-center gap-3">
                    ...
                    {card.active && (
                      <Button
                        ...
                        onClick={() => {
                          if (confirm(`Void gift card ${card.code}? This cannot be undone.`)) {
