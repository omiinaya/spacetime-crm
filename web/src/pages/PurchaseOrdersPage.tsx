...
const statusColors: Record<
  string,
  'outline' | 'default' | 'success' | 'destructive' | 'secondary'
> = {
  draft: 'outline',
...
export default function PurchaseOrdersPage() {
  ...
  const addItemMutation = useMutation({
    ...
    onError: (err) =>
      toast.error(
        err instanceof Error && err.message === 'Description required'
          ? err.message
              ...
              <CardContent className="pt-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {po.currency || 'USD'} {po.total.toFixed(2)}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  {selectedPo === po.id ? (
            ...
            <>
              ...
              <div className="flex gap-2 mb-4 flex-wrap">
                ...
                {(poDetail.status === 'sent' || poDetail.status === 'partial') && (
                  <Button
                    size="sm"
                    onClick={() => setReceiveMode(receiveMode === poDetail.id ? null : poDetail.id)}
                  >
                    <PackageCheck className="h-3.5 w-3.5 mr-1.5" />
              ...
              <Card className="mb-4">
                <CardContent className="pt-4 space-y-1 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span>
                      {poDetail.currency || 'USD'} {poDetail.subtotal.toFixed(2)}
                    </span>
                    {poDetail.shipping_cost > 0 && (
                      <div className="flex justify-between text-muted-foreground">
                        <span>Shipping</span>
                        <span>
                          {poDetail.currency || 'USD'} {poDetail.shipping_cost.toFixed(2)}
                        </span>
                      </div>
                    )}
                  </div>
                  <div className="flex justify-between font-bold text-base border-t pt-1 mt-1">
                    <span>Total</span>
                    <span>
                      {poDetail.currency || 'USD'} {poDetail.total.toFixed(2)}
                    </span>
                  </div>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  {poDetail.status !== 'received' && poDetail.status !== 'cancelled' && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => setNewItemForm({ ...newItemForm, description: '' })}
                    >
                      ...
                      <CardContent className="pt-3">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-muted-foreground">
                              {item.quantity} × {poDetail.currency || 'USD'}{' '}
                              {item.unit_price.toFixed(2)} = {poDetail.currency || 'USD'}{' '}
                            ...
                            <div className="flex items-center gap-2 text-xs text-muted-foreground mt-1">
                              ...
                              <div className="h-1.5 w-20 bg-muted rounded-full overflow-hidden">
                                <div
                                  ...
                                  style={{
                                    width: `${Math.min(100, (item.received_quantity / item.quantity) * 100)}%`,
                                  }}
                                />
                              </div>
                            </div>
                ...
                <Card className="border-green-500/30 bg-green-500/5">
                  <CardContent className="pt-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button size="sm" variant="outline" onClick={() => setReceiveMode(null)}>
                        Cancel
                      </Button>
                    </div>
                  </CardContent>
