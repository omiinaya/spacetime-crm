...
const PortalEstimatesPage = lazy(() => import('./pages/PortalEstimatesPage'));
...
type PortalPage = 'dashboard' | 'tickets' | 'invoices' | 'estimates' | 'appointments';
    ...
    <nav className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto py-2 px-2">
        {navSections
          .map((section) => ({
            ...section,
            items: section.items.filter((item) => isNavVisible(item.id, user?.role || '')),
          }))
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 space-y-6">
          {(() => {
            ...
            if (!tabs) return null;
            const visible = tabs.filter((t) => isNavVisible(t.id, user?.role || ''));
            if (visible.length <= 1) return null;
            const active = visible.some((t) => t.id === page) ? page : visible[0].id;
            return (
              <Tabs tabs={visible} active={active} onChange={(id) => navigate(id as PageId)} />
            );
          })()}
          {!online && (
            <div
            ...
            >
              ...
              <span>
                You're offline — showing cached data. Changes will be saved when your connection
                returns.
              </span>
            </div>
          )}
