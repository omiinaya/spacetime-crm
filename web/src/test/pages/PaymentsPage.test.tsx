...
describe("PaymentsPage", () => {
	it("renders page header and payments from API", async () => {
		...
		await waitFor(() => {
			...
			expect(screen.getAllByText(/USD 100\.00/).length).toBeGreaterThanOrEqual(
				1,
			);
		});
	});
	...
	it("opens record payment form", async () => {
		...
		const recordBtn = screen.getAllByRole("button", {
	...
	it("records a payment", async () => {
		...
		await waitFor(() => {
			...
			const post = calls.find(
				(c) => c.init?.method === "POST" && c.url.includes("/payments"),
			);
	...
	it("deletes a payment", async () => {
		...
		await waitFor(() => {
			...
			expect(
				calls.some(
					(c) =>
						c.init?.method === "DELETE" && c.url.includes("/payments/pmt_1"),
				),
			).toBeTruthy();
		});
	});
