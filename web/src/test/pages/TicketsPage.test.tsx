...
function pushPage(customers: unknown[], tickets: unknown[], total = tickets.length) {
...
describe('TicketsPage', () => {
  ...
  it('creates a new ticket with form data', async () => {
    ...
    await waitFor(() => {
      ...
      const post = calls.find((c) => c.init?.method === 'POST' && c.url.includes('/tickets'));
  ...
  it('filters tickets by status', async () => {
    ...
    await waitFor(() => {
      const calls = mock.calls();
      expect(
        calls.some((c) => c.url.includes('/tickets') && c.url.includes('status=in_progress')),
      ).toBeTruthy();
    });
  });
});
