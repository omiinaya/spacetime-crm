...
export const giftCards = {
	list: (offset = 0, limit = 50, active?: string) => {
		...
		return apiFetch<{ gift_cards: GiftCard[]; total: number }>(
			`/gift-cards?${p}`,
		);
	},
	...
	lookup: (code: string) =>
		apiFetch<{ gift_card: GiftCard }>(
			`/gift-cards/lookup?code=${encodeURIComponent(code)}`,
		),

	redeem: (data: GiftCardRedeemPayload) =>
		apiFetch<{ ok: boolean; redeemed: number; remaining: number }>(
			"/gift-cards/redeem",
			{
				method: "POST",
				body: JSON.stringify(data),
