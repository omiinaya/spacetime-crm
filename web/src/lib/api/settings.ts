...
import type { MailSettings, SmsSettings, BusinessHours, TaxRate } from './types';
...
export const settings = {
  ...
  app: {
    get: () => apiFetch<{ config: { revenue_target: number } }>('/settings/app'),
    save: (data: { revenue_target: number }) =>
      apiFetch<{ ok: boolean; config: { revenue_target: number } }>('/settings/app', {
        method: 'POST',
        body: JSON.stringify(data),
      }),
  },
