...
export const api = {
	...
	customers: {
		...
		geolocations: {
			list: () =>
				apiFetch<{ locations: CustomerGeoLocation[] }>(
			...
			geocodeAll: () =>
				apiFetch<{ geocoded: number; results: any[] }>(
	...
	products: {
		...
		lowStock: {
			...
			notify: () =>
				apiFetch<{ ok: boolean; count?: number; notified?: string }>(
		...
		transfer: (data: {
			...
			apiFetch<{ ok: boolean; quantity: number; reference: string }>(
				"/products/transfer",
				{
					method: "POST",
					body: JSON.stringify(data),
	...
	webhooks: {
		list: () =>
			apiFetch<{ subscriptions: WebhookSubscription[] }>(
