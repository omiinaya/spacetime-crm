...
async def get_overdue_count(user: dict = Depends(require_role("admin", "tech", "front_desk"))):
    ...
    rows = await _sql(
        f"SELECT * FROM invoices WHERE tenant_id = '{user['tenant_id']}' AND (status = 'overdue' OR ((status = 'sent' OR status = 'partial') AND due_date > 0 AND due_date < {int(datetime.now(timezone.utc).timestamp() * 1000)}))"
    )
...
async def send_overdue_reminders(user: dict = Depends(require_role("admin"))):
    ...
    for inv in rows:
        cust = await _sql(f"SELECT * FROM customer WHERE id = '{_safe_id(inv['customer_id'])}'")
        if not cust:
            continue
        c = cust[0]
...
async def invoice_pdf(invoice_id: str, user: dict = Depends(require_role("admin", "tech", "front_desk"))):
    ...
    html = template.render(
        ...
        if status not in ("paid", "partial")
            ...
            {
                "date": datetime.fromtimestamp(float(p.get("created_at", 0)) / 1000).strftime("%b %d, %Y")
                if p.get("created_at")
                else "—",
                "method": p.get("method", "—"),
...
async def send_invoice_email(
    ...
    await _log_audit(user, "send_email", "invoice", invoice_id, f"to={customer_email} invoice={inv_num}")
