...
async def get_invoice_summary(
    ...
    sent_partial_overdue = sum(
        ...
        and i.get("due_date", 0) > 0
        and i.get("due_date", 0) < now
    )
    sent_partial_overdue_total = round(
        sum(
            float(i.get("total", 0))
            for i in rows
            if i.get("status") in ("sent", "partial")
    ...
    return {
        ...
        "total_revenue": round(
            sum(float(i.get("total", 0)) for i in rows if i.get("status") == "paid"), 2
        ),
...
async def bulk_update_invoice_status(
    body: BulkInvoiceStatusUpdate, user: dict = Depends(require_role("admin"))
):
...
async def bulk_edit_invoices(body: BulkInvoiceEdit, user: dict = Depends(require_role("admin"))):
    ...
    for inv_id in body.invoice_ids:
        try:
            ...
            logger.error("bulk_edit_invoices: update failed for invoice %s", inv_id)
            errors += 1
    if updated:
        await _log_audit(
            user,
            ...
            "invoice",
            f"count={updated}",
            f"notes={has_notes} terms={has_terms}",
        )
    return {"ok": True, "updated": updated, "errors": errors}
...
async def get_overdue_count(
    ...
    rows = await _sql(
        f"SELECT * FROM invoices WHERE tenant_id = '{user['tenant_id']}' AND (status = 'overdue' OR ((status = 'sent' OR status = 'partial') AND due_date > 0 AND due_date < {int(datetime.utcnow().timestamp() * 1000)}))"
    )
