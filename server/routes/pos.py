...
async def list_pos_sales(offset: int = 0, limit: int = 50, user: dict = Depends(require_role("admin", "tech", "front_desk"))):
    """List counter sales with pagination."""
    rows, total = await _paginated(
        user["tenant_id"], "counter_sale",
...
async def create_pos_sale(body: POSCreate, user: dict = Depends(require_role("admin", "tech", "front_desk"))):
    ...
    await _call("create_counter_sale", [
        user["tenant_id"],
        body.customer_id,
        body.customer_name,
        body.payment_method,
...
async def add_pos_item(body: POSAddItem, user: dict = Depends(require_role("admin", "tech", "front_desk"))):
    ...
    await _call("add_counter_sale_item", [
        user["tenant_id"],
        body.sale_id,
        body.product_id,
        body.product_name,
...
async def list_receipts(offset: int = 0, limit: int = 50, user: dict = Depends(require_role("admin", "tech", "front_desk"))):
    """List completed sales sorted by receipt number."""
    rows, total = await _paginated(
        user["tenant_id"], "counter_sale",
        offset=offset, limit=limit,
        where_extra="status = 'completed'",
        order_by="receipt_number", order_desc=True,
    )
