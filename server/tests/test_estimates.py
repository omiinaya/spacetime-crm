...
def _create_customer(test_admin_headers: dict, suffix: str = "") -> str:
    suf = suffix or unique_suffix()
    c = create_customer(test_admin_headers, first_name="Est", last_name=f"Test{suf}", email=f"est-{suf}@example.com")
    return c["id"]


def _create_estimate(test_admin_headers: dict, suffix: str = "") -> str:
    cid = _create_customer(test_admin_headers, suffix)
...
class TestEstimateCRUD:
    ...
    def test_create_estimate(self, test_admin_headers: dict):
        cid = _create_customer(test_admin_headers, "create")
    ...
    def test_add_line_items(self, test_admin_headers: dict):
        est_id = _create_estimate(test_admin_headers, "lineitems")
        ...
        r = httpx.get(f"{SERVER_URL}/api/estimates/{est_id}/line-items", headers=test_admin_headers, timeout=10)
        data = assert_ok(r)
        assert len(data["line_items"]) >= 2

    def test_update_status(self, test_admin_headers: dict):
        est_id = _create_estimate(test_admin_headers, "status")
    ...
    def test_delete_estimate(self, test_admin_headers: dict):
        est_id = _create_estimate(test_admin_headers, "delete")
...
class TestEstimateConversion:
    ...
    def test_convert_approved_estimate(self, test_admin_headers: dict):
        """Full conversion: create estimate → approve → convert → get invoice."""
        est_id = _create_estimate(test_admin_headers, "convert")
        cid = _create_customer(test_admin_headers, "conv2")
        ...
        r2 = httpx.get(f"{SERVER_URL}/api/invoices", params={"limit": 500}, headers=test_admin_headers, timeout=10)
        inv_ids = [inv["id"] for inv in r2.json().get("invoices", [])]
        assert invoice_id in inv_ids, f"Invoice {invoice_id} not found in list"

    def test_convert_non_approved_rejected(self, test_admin_headers: dict):
        """Only approved estimates can be converted."""
        est_id = _create_estimate(test_admin_headers, "noconvert")
