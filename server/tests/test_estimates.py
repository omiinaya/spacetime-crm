...
class TestEstimateCRUD:
    ...
    def test_estimate_pdf_generation(self, test_admin_headers: dict, session_suffix: str):
        """PDF endpoint for estimates returns valid application/pdf content."""
        est_id = _create_estimate(test_admin_headers, session_suffix, "pdf")
        httpx.post(
            f"{SERVER_URL}/api/estimates/{est_id}/line-items",
            json={"description": "Labor - repair", "quantity": 2, "unit_price": 65},
            headers=test_admin_headers,
            timeout=10,
        )
        resp = httpx.get(
            f"{SERVER_URL}/api/estimates/{est_id}/pdf",
            headers=test_admin_headers,
            timeout=15,
        )
        ...
        assert resp.content.startswith(b"%PDF"), "PDF magic bytes not present"
        assert len(resp.content) > 1000, f"PDF too small: {len(resp.content)} bytes"

    def test_estimate_pdf_nonexistent(self, test_admin_headers: dict):
        resp = httpx.get(
            f"{SERVER_URL}/api/estimates/nonexistent-id-99999/pdf",
            headers=test_admin_headers,
            timeout=10,
        )
