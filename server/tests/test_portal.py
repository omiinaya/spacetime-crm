...
Each test creates its own data where needed — no inter-test order dependencies.
All created entities are tracked for session cleanup.
Uses session_suffix isolation so parallel test runs never collide.
"""
import bcrypt
import httpx
import pytest
...
def _create_portal_ticket(
    customer_id: str, admin_headers: dict, tag: str = ""
) -> str:
...
def _create_portal_invoice(customer_id: str, admin_headers: dict, tag: str = "") -> str:
    ...
    resp = httpx.post(
        ...
        json={"customer_id": customer_id, "notes": notes, "due_date": 0},
        headers=admin_headers,
        timeout=10,
    )
    data = assert_ok(resp)
...
class TestPortalTickets:
    ...
    def test_ticket_detail(
        ...
        resp = httpx.get(
            f"{SERVER_URL}/api/portal/tickets/{tid}",
...
class TestPortalAppointments:
    ...
    def test_list_appointments(self, portal_headers: dict):
        resp = httpx.get(
            f"{SERVER_URL}/api/portal/appointments",
            headers=portal_headers,
            timeout=10,
        )
        data = assert_ok(resp)
...
class TestPortalErrors:
    ...
    def test_admin_token_rejected(self):
        ...
        resp = httpx.get(
            f"{SERVER_URL}/api/portal/me", headers=headers, timeout=10
        )
        assert resp.status_code == 401, (
