...
class TestPOSCRUD:
    ...
    def test_get_nonexistent(self, test_admin_headers: dict):
        resp = httpx.get(
            f"{SERVER_URL}/api/pos/sales/nonexistent-999",
            headers=test_admin_headers,
            timeout=10,
        )
...
class TestPOSItems:
    ...
    def test_item_line_item_ordering(
        self, test_admin_headers: dict, test_product_id: str, session_suffix: str
    ):
...
class TestPOSRefund:
    ...
    def test_refund_sale(self, test_admin_headers: dict, session_suffix: str):
        ...
        resp = httpx.post(
            f"{SERVER_URL}/api/pos/refund/{sale_id}",
            headers=test_admin_headers,
            timeout=10,
        )
        assert_ok(resp)
...
class TestPOSReceiptPdf:
    ...
    def test_receipt_pdf_returns_pdf(
        ...
        resp = httpx.get(
            f"{SERVER_URL}/api/pos/sales/{sale_id}/receipt-pdf",
            headers=test_admin_headers,
            timeout=10,
        )
    ...
    def test_receipt_pdf_has_content_disposition(
        self, test_admin_headers: dict, session_suffix: str
    ):
