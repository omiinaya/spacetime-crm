...
class TestProductCRUD:
    ...
    def test_update_product_quantity(
        ...
        httpx.post(
            f"{SERVER_URL}/api/products",
            json={"name": "Qty Test", "sku": sku, "price": 10, "quantity_on_hand": 25},
            headers=test_admin_headers,
            timeout=10,
        )
        r = httpx.get(
    ...
    def test_delete_product(self, test_admin_headers: dict, session_suffix: str):
        ...
        httpx.post(
            f"{SERVER_URL}/api/products",
            json={"name": "Delete Test", "sku": sku, "price": 5, "quantity_on_hand": 0},
            headers=test_admin_headers,
            timeout=10,
        )
        r = httpx.get(
        ...
        resp = httpx.delete(
            f"{SERVER_URL}/api/products/{pid}", headers=test_admin_headers, timeout=10
        )
        assert_ok(resp)
    ...
    def test_inventory_adjustment(self, test_admin_headers: dict, session_suffix: str):
        ...
        httpx.post(
            f"{SERVER_URL}/api/products",
            json={"name": "Adj Test", "sku": sku, "price": 8, "quantity_on_hand": 30},
            headers=test_admin_headers,
            timeout=10,
        )
        r = httpx.get(
        ...
        resp = httpx.post(
            ...
            json={
                "quantity_change": -5,
                "reason": "damaged",
                "notes": "Screen cracked in storage",
            },
            headers=test_admin_headers,
            timeout=10,
        )
    ...
    def test_list_adjustments(self, test_admin_headers: dict, session_suffix: str):
        ...
        httpx.post(
            f"{SERVER_URL}/api/products",
            json={"name": "List Adj", "sku": sku, "price": 12, "quantity_on_hand": 20},
            headers=test_admin_headers,
            timeout=10,
        )
        r = httpx.get(
        ...
        httpx.post(
            f"{SERVER_URL}/api/products/{pid}/adjustments",
            json={"quantity_change": 10, "reason": "restock", "notes": "New shipment"},
            headers=test_admin_headers,
            timeout=10,
        )
    ...
    def test_low_stock_list(self, test_admin_headers: dict, session_suffix: str):
        ...
        resp = httpx.get(
            f"{SERVER_URL}/api/products/low-stock",
            headers=test_admin_headers,
            timeout=10,
        )
        data = assert_ok(resp)
    ...
    def test_low_stock_notify(self, test_admin_headers: dict):
        ...
        resp = httpx.post(
            f"{SERVER_URL}/api/products/low-stock/notify",
            headers=test_admin_headers,
            timeout=10,
        )
        data = assert_ok(resp)
...
class TestProductErrors:
    ...
    def test_unauthorized_access(self, client: httpx.Client):
        ...
        for path in [
            "/api/products",
            "/api/products/fake/adjustments",
            "/api/products/low-stock",
            "/api/products/by-barcode/test123",
        ]:
...
class TestBarcodeLookup:
    ...
    def test_lookup_by_barcode(self, test_admin_headers: dict, session_suffix: str):
        ...
        if prods:
            _track_entity("product", prods[0]["id"])
        resp = httpx.get(
            f"{SERVER_URL}/api/products/by-barcode/{barcode}",
            headers=test_admin_headers,
            timeout=10,
        )
        data = assert_ok(resp)
