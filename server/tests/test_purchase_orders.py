...
def _create_po(test_admin_headers: dict, session_suffix: str, tag: str = "") -> str:
    ...
    """
    suf = (
        f"{session_suffix}-{tag}-{unique_suffix()}"
        if tag
        else f"{session_suffix}-{unique_suffix()}"
    )
    vendor = f"Vendor-{suf}"
    httpx.post(
...
class TestPurchaseOrderCRUD:
    ...
    def test_list_purchase_orders(self, test_admin_headers: dict):
        resp = httpx.get(
            f"{SERVER_URL}/api/purchase-orders", headers=test_admin_headers, timeout=10
        )
        data = assert_ok(resp)
...
class TestPOReceiving:
    ...
    def test_receive_item_updates_stock(
        ...
        for li in r2.json()["purchase_order"]["line_items"]:
            if li["id"] == item_id:
                assert float(li.get("received_quantity", 0)) == 10, (
        ...
        assert any(a.get("reason") == "received" for a in adj), (
    ...
    def test_full_receive_updates_progress(
        ...
        for li in po_data["line_items"]:
            if li["id"] == item_id:
                assert float(li.get("received_quantity", 0)) == 5, (
