...
class TestWebhookCRUD:
    ...
    def test_create_missing_url(self, test_admin_headers: dict):
        resp = httpx.post(f"{SERVER_URL}/api/webhook-subscriptions", json={"events": "customer.created"}, headers=test_admin_headers, timeout=10)
    ...
    def test_delete_nonexistent(self, test_admin_headers: dict):
        resp = httpx.delete(f"{SERVER_URL}/api/webhook-subscriptions/nonexistent-999", headers=test_admin_headers, timeout=10)
...
class TestWebhookTest:
    ...
    def test_test_nonexistent(self, test_admin_headers: dict):
        resp = httpx.post(f"{SERVER_URL}/api/webhook-subscriptions/nonexistent-999/test", headers=test_admin_headers, timeout=10)
...
class TestWebhookStripe:
    ...
    def test_stripe_webhook_no_signature(self, test_admin_headers: dict):
        ...
        resp = httpx.post(f"{SERVER_URL}/api/webhooks/stripe", json={"type": "checkout.session.completed", "data": {"object": {}}}, headers=test_admin_headers, timeout=10)
        assert resp.status_code == 400
