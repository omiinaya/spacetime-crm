...
class TestTaxRates:
    def test_list_tax_rates(self, client, monkeypatch):
    ...
    def test_list_tax_rates_unauthorized(self, client):
        resp = client.get("/api/tax-rates")
        assert resp.status_code == 401

    def test_create_tax_rate(self, client, monkeypatch):
    ...
    def test_update_tax_rate(self, client, monkeypatch):
        mock_call = AsyncMock(return_value={})
        monkeypatch.setattr("routes.tax_rates._call", mock_call)
        monkeypatch.setattr("routes.tax_rates._log_audit", AsyncMock())
    ...
    def test_delete_tax_rate(self, client, monkeypatch):
        mock_call = AsyncMock(return_value={})
        monkeypatch.setattr("routes.tax_rates._call", mock_call)
        monkeypatch.setattr("routes.tax_rates._log_audit", AsyncMock())
