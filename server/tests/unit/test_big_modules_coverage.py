...
def test_invoices_list(client, auth_headers, configure_stdb):
    ...
    resp = client.get("/api/invoices", headers=auth_headers); assert resp.status_code != 500
...
def test_invoices_summary(client, auth_headers, configure_stdb):
    ...
    resp = client.get("/api/invoices/summary", headers=auth_headers); assert resp.status_code != 500

def test_invoices_overdue(client, auth_headers, configure_stdb):
    configure_stdb(json_data=[{"count": 5}], status=200)
    resp = client.get("/api/invoices/overdue-count", headers=auth_headers); assert resp.status_code != 500
...
def test_invoices_line_items(client, auth_headers, configure_stdb):
    ...
    resp = client.get("/api/invoices/i1/line-items", headers=auth_headers); assert resp.status_code != 500
...
def test_invoices_delete(client, auth_headers, configure_stdb):
    ...
    resp = client.delete("/api/invoices/i1", headers=auth_headers); assert resp.status_code != 500

def test_invoices_pdf(client, auth_headers, configure_stdb):
    configure_stdb(json_data=[{"url": "test.pdf"}], status=200)
    resp = client.get("/api/invoices/i1/pdf", headers=auth_headers); assert resp.status_code != 500
...
def test_portal_me(client, auth_headers, configure_stdb):
    configure_stdb(json_data=[{"id": "u1"}], status=200)
    resp = client.get("/api/portal/me", headers=auth_headers); assert resp.status_code != 500

def test_portal_stats(client, auth_headers, configure_stdb):
    configure_stdb(json_data=[{"total_tickets": 10}], status=200)
    resp = client.get("/api/portal/stats", headers=auth_headers); assert resp.status_code != 500

def test_portal_tickets(client, auth_headers, configure_stdb):
    configure_stdb(json_data=[{"id": "t1"}], status=200)
    resp = client.get("/api/portal/tickets", headers=auth_headers); assert resp.status_code != 500
    resp = client.get("/api/portal/tickets/t1", headers=auth_headers); assert resp.status_code != 500

def test_portal_invoices(client, auth_headers, configure_stdb):
    configure_stdb(json_data=[{"id": "pi1"}], status=200)
    resp = client.get("/api/portal/invoices", headers=auth_headers); assert resp.status_code != 500
...
def test_portal_appointments(client, auth_headers, configure_stdb):
    ...
    resp = client.get("/api/portal/appointments", headers=auth_headers); assert resp.status_code != 500
...
def test_dashboard_full(client, auth_headers, configure_stdb):
    ...
    resp = client.get("/api/dashboard", headers=auth_headers); assert resp.status_code != 500

def test_dashboard_stats(client, auth_headers, configure_stdb):
    configure_stdb(json_data=[{"total": 100}], status=200)
    resp = client.get("/api/dashboard/stats", headers=auth_headers); assert resp.status_code != 500
    resp = client.get("/api/dashboard/revenue", headers=auth_headers); assert resp.status_code != 500

# ── Export/Import (96 stmts, 19%) ──

def test_export_full(client, auth_headers, configure_stdb):
    configure_stdb(json_data=[{"id": "1"}], status=200)
    resp = client.get("/api/export/customers", headers=auth_headers); assert resp.status_code != 500
    resp = client.get("/api/export/products", headers=auth_headers); assert resp.status_code != 500
