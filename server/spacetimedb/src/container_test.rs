...
mod tests {
    ...
    fn stdb_url() -> String {
        env::var("STDB_CONTAINER_URL").unwrap_or_else(|_| "http://localhost:3002".to_string())
    }

    fn stdb_db() -> String {
        env::var("STDB_CONTAINER_DB").unwrap_or_else(|_| "spacetime-crm-test".to_string())
    }
    ...
    fn run_curl(method: &str, url: &str, data: Option<&str>) -> String {
        ...
        let output = cmd
            .output()
            .unwrap_or_else(|e| panic!("curl failed (is curl installed?): {e}"));
        assert!(
    ...
    fn container_module_is_published() {
        ...
        assert!(
            out.contains("cnt") || out.contains("rows"),
            "Expected customer table to exist, got: {:.200}",
            out
        );
    }
    ...
    fn container_list_tables() {
        ...
        for tbl in &[
            ...
            assert!(
                out.contains("cnt") || out.contains("rows"),
                "Table '{tbl}' should exist, got: {:.200}",
                out
            );
        }
    }
    ...
    fn container_create_ticket() {
        ...
        assert!(
            out.contains("new"),
            "Status should be 'new', got: {:.200}",
            out
        );
        assert!(
            out.contains("high"),
            "Priority should be 'high', got: {:.200}",
            out
        );
    }
    ...
    fn container_update_ticket_status() {
        ...
        let out = run_sql(&format!("SELECT status FROM ticket WHERE id = '{}'", id));
        assert!(
            out.contains("in_progress"),
            "Status should be 'in_progress', got: {:.200}",
            out
        );
    }
    ...
    fn container_create_invoice() {
        ...
        let out =
            run_sql("SELECT status, currency FROM invoice WHERE notes = 'Container invoice test'");
        assert!(
            out.contains("draft"),
            "Status should be 'draft', got: {:.200}",
            out
        );
        assert!(
            out.contains("USD"),
            "Currency should be USD, got: {:.200}",
            out
        );
    }
    ...
    fn container_record_payment() {
        ...
        let out = run_sql("SELECT amount, method FROM payment WHERE reference = 'REF-001'");
        assert!(
            out.contains("150"),
            "Amount should be 150, got: {:.200}",
            out
        );
        assert!(
            out.contains("cash"),
            "Method should be 'cash', got: {:.200}",
            out
        );
    }
    ...
    fn container_create_user() {
        ...
        let out = run_sql("SELECT name, role, active FROM user WHERE email = 'container@user.com'");
        assert!(
            out.contains("container_user"),
            "User should exist, got: {:.200}",
            out
        );
        assert!(
            out.contains("tech"),
            "Role should be 'tech', got: {:.200}",
            out
        );
        assert!(
            out.contains("true") || out.contains("1"),
            "User should be active, got: {:.200}",
            out
        );
    }
    ...
    fn container_create_tenant() {
        ...
        let out = run_sql("SELECT name FROM tenant WHERE slug = 'container-shop'");
        assert!(
            out.contains("Container Shop"),
            "Tenant should exist, got: {:.200}",
            out
        );
    }
    ...
    fn container_create_appointment() {
        ...
        let out =
            run_sql("SELECT status FROM appointment WHERE title = 'Screen repair - container'");
        assert!(
            out.contains("scheduled"),
            "Status should be 'scheduled', got: {:.200}",
            out
        );
    }
}
