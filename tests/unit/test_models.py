...
from models import (
    ...
    InvoiceCreate, InvoiceStatusUpdate, InvoiceLineItemCreate, BulkInvoiceStatusUpdate,
    PaymentCreate,
    AppointmentCreate, AppointmentStatusUpdate,
    ProductCreate, ProductQuantityUpdate,
    PurchaseOrderCreate, PurchaseOrderStatusUpdate, POLineItemCreate,
    ...
    ChecklistTemplateCreate, ChecklistToggle,
    WebhookSubscriptionCreate, WebhookSubscriptionUpdate,
    ScheduledReportCreate, ScheduledReportUpdate,
    RecurringInvoiceRuleCreate, DayHours, BusinessHoursUpdate,
    SavePaymentMethodRequest, Setup2FARequest, PortalLoginRequest, POSCreate,
)
...
class TestCustomerModels:
    ...
    def test_customer_update_with_all_fields(self):
        r = CustomerUpdate(
            ...
            email="j@ex.com", phone="555-1234", mobile="555-5678",
            company="Acme", address_line1="123 Main", city="NYC",
            state="NY", zip="10001", notes="VIP", tags="important",
        )
...
class TestTicketModels:
    ...
    def test_ticket_create_with_all_fields(self):
        r = TicketCreate(
            customer_id="c1", title="Fix PC",
            description="Blue screen", device_type="laptop",
            device_model="XPS 15", device_serial="SN123",
        )
...
class TestInvoiceModels:
    ...
    def test_invoice_create_with_all(self):
        r = InvoiceCreate(
            customer_id="c_1", ticket_id="t_1",
            notes="Thank you", terms="Net 30",
            due_date=1700000000000, discount_amount=10, discount_percent=5,
        )
...
class TestAppointmentModels:
    def test_appointment_create_valid(self):
        r = AppointmentCreate(customer_id="c_1", title="Screen Repair", start_time=1700000000000, end_time=1700003600000)
...
class TestTenantModels:
    ...
    def test_tenant_update(self):
        r = TenantUpdate(name="Shop One Updated", slug="shop-one", logo_url="https://example.com/logo.png", settings="{}")
...
class TestScheduleModels:
    def test_scheduled_report_create(self):
        r = ScheduledReportCreate(
            name="Weekly Report", report_type="revenue",
            schedule_frequency="weekly", recipients=["admin@ex.com"],
        )
    ...
    def test_scheduled_report_invalid_type(self):
        ...
        r = ScheduledReportCreate(
            name="Bad", report_type="invalid",
            schedule_frequency="weekly", recipients=["a@b.com"],
        )
        assert r.report_type == "invalid"

    def test_scheduled_report_update(self):
        r = ScheduledReportUpdate(
            name="Monthly", report_type="tickets",
            schedule_frequency="monthly", recipients=["a@b.com"], enabled=False,
        )
        assert r.enabled is False
